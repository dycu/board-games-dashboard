...
const METATAGS_URL = 'https://www.yucata.de/api/gameinfo/metatags'
...
export async function fetchYucataCatalog(): Promise<CatalogEntry[]> {
  const gamesRes = await fetch(METATAGS_URL, {
    headers: { Accept: 'application/json', 'User-Agent': 'Mozilla/5.0' },
  })
  if (!gamesRes.ok) throw new Error(`Yucata metatags fetch failed: HTTP ${gamesRes.status}`)
